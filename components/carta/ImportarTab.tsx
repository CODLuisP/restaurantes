...
export default function ImportarTab() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      resizedBlob = await resizeImageToBlob(file, 800, 800, 0.75);
    ...
    try {
      ...
      body.append('image', resizedBlob || file, 'plato.jpg');
            ...
            <div className="relative aspect-4/3 sm:aspect-square w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
              ...
              {status === 'analyzing' && (
                <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center gap-2 text-white">
                  <Spinner size="lg" />
                  <span className="text-xs font-semibold text-white">Analizando con IA...</span>
                </div>
              )}
            </div>
            <button
              ...
              disabled={guardando}
              className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 py-2 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
