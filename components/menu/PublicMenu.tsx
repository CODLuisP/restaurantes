...
function productoToCartItem(p: ProductoMenu): Product {
  return {
    id: String(p.id),
    name: p.nombre,
    price: p.precio,
...
export default function PublicMenu({
  ...
  const [horarios, setHorarios] = useState({ zonaHoraria: 'Peru (Lima)', tipoNegocio: 'Restaurante', descripcionCompleta: '', whatsappPedidos: '', schedule: {} as Record<string, any> });
  ...
  const [orderType, setOrderType] = useState<"mesa" | "llevar" | "delivery">(
    mesaLabel ? "mesa" : "llevar",
  );
  ...
  const [tableNum, setTableNum] = useState(
    mesaLabel ? mesaLabel.replace(/Mesa /i, "") : "",
  );
  ...
  const handleConfirmOrder = () => {
    ...
    try {
      ...
      if (orderType === "mesa") {
        ...
        const tablesList: Table[] = storedTables
        ...
        if (tableIndex !== -1) {
          ...
          cart.forEach((cartItem) => {
            const idx = mergedItems.findIndex(
              (mi) => mi.product.id === cartItem.product.id,
            );
            if (idx !== -1) {
          ...
          const cuenta = mergedItems.reduce(
            (acc, i) => acc + i.product.price * i.quantity,
            0,
          );
          ...
          const activeList: ActiveOrder[] = storedActive
          ...
          const newOrder: ActiveOrder = {
            ...
            paymentMethod,
            paymentScreenshot:
              paymentMethod === "Yape / Plin" ? paymentScreenshot : undefined,
          };

          activeList.push(newOrder);
          localStorage.setItem(
            "restopro.activeOrders",
        ...
        const kitchenList: KitchenOrder[] = storedKitchen
          ? JSON.parse(storedKitchen)
          : [];
        ...
        localStorage.setItem(
          "restopro.kitchenOrders",
          JSON.stringify(kitchenList),
        );
      } else {
        // Llevar o Delivery
        const storedActive = localStorage.getItem("restopro.activeOrders");
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-2 w-full">
            ...
            <Button className="flex-1 py-3" onClick={handleConfirmOrder}>
              Confirmar y Enviar
            </Button>
          </div>
        }
